...
const AnimatedText = () => {
  ...
  return (
    <div className="glitch-wrapper mb-4">
      <h1 className="text-3xl sm:text-4xl font-bold glitch layers text-center sm:text-left" data-text={displayText}>
        <span>{displayText.split('').map((char, index) => (
          <span key={index} className={index >= displayText.length - (finalText.length - displayText.replace(/[ｱ-ﾝ]/g, '').length) ? `${inter.className}` : ''}>
            {char}
          </span>
        ))}</span>
      </h1>
